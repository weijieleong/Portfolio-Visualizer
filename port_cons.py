...
def calculate_tracking_error(portfolio_returns, benchmark_returns, freq='weekly'):
...
def performance_summary(portfolio_returns, portfolio_weights, benchmark_rel_perf, benchmark_returns=None, start_date=None, end_date=None, frequency='weekly', rebal_per_year=1):
    if frequency=='daily':
        annualized = 261
    if frequency=='weekly':
        annualized = 52
...
def performance_summary_yearly(*args, freq='weekly', rf=0):
...
def performance_summary_constituents(returns, start_date=None, end_date=None, frequency='weekly'):
...
def summary_returns_constituents(returns_df, frequency='weekly', rf=0):
...
def summary_vol_constituents(returns_df, frequency='weekly', rf=0):
    returns_df.index = pd.to_datetime(returns_df.index, format='%Y%m%d')
    results = {}
...
def plot_return_risk_scatter_year(returns_df, freq='weekly', periods=[1, 5], show_data=True, width=1500, height=800):
